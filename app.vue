  ...
  async () => {
    ...
    try {
      response = await $fetch(
        `http://api.weatherapi.com/v1/current.json?key=db7df8852e3642f9a9e10323230202&q=${search.value}`);
  ...
  <div class="h-screen relative overflow-hidden">
    ...
    <div class="absolute w-full h-full top-0 p-48">
      <div class="flex justify-between">
        <div>
          <h1 class="text-7xl text-white">{{ data.location.name }}</h1>
          <p class="font-extralight text-2xl mt-2 text-white">Sunday Dec 9th</p>
          <img :src="data.current.condition.icon" class="w-56 icon" />
        </div>
        <div>
      ...
      <div class="mt-20">
        <input type="text" class="w-1/2 h-10" placeholder="Search a data.city..." v-model="input" />
